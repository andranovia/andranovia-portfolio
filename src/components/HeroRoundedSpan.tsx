  ...
  return (
    <div>
      ...
      {RoundedSpanData.map((data, index) => (
        <div
          className={index === 0 ? `rounded-full border-2 flex justify-start gap-2 items-center border-white bg-black text-white px-4 py-2` : `rounded-full border-2 flex justify-start gap-2 items-center border-gray-600 px-4 py-2`}
          key={index}
        >
          <p className="text-start text-sm sm:text-base">{data.text}</p>
            <Image src={data.image} alt="" width={30} height={30} className="w-6"/>
        </div>
      ))}
      </div>
    </div>
